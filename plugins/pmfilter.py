...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn = [[
            ...
            InlineKeyboardButton("🤧 Not Available In Your Requested Language",
        ...
        btn = [[
            InlineKeyboardButton(
                "🤧 Not Available In Your Requested Language", callback_data=f"hnalert#{from_user}")
        ]]
        btn2 = [[
            InlineKeyboardButton('ᴊᴏɪɴ ᴄʜᴀɴɴᴇʟ', url=link.invite_link),
            InlineKeyboardButton("ᴠɪᴇᴡ ꜱᴛᴀᴛᴜꜱ", url=f"{query.message.link}")
        ...
        buttons = [[
                InlineKeyboardButton('Gpt Learning Suggest', url=f'https://www.google.com/')
            ],[
        ...
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto('https://graph.org/file/762337ddbfefe4ef5fb96-0468bf018693d08e64.jpg')
        )
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        if settings.get('button'):
            cap = f"<b>✍️ Name: <code>{search}</code>\n🗺️ Total Movies/Series : <code>{total_results}</code>\n👣 Timing : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n\n🙏 Movie Requested By : {message.from_user.mention}\n🦚 Powered By :  {message.chat.title or temp.B_LINK or 'ᴅʀᴇᴀᴍxʙᴏᴛᴢ'} \n\n🧾 <u>Your Requested Files Are Here</u> 👇 \n\n</b>"
        else:
            cap = f"<b>✍️ Name: <code>{search}</code>\n🗺️ Total Movies/Series : <code>{total_results}</code>\n👣 Timing : <code>{remaining_seconds} Sᴇᴄᴏɴᴅs</code>\n\n🙏 Movie Requested By : {message.from_user.mention}\n🦚 Powered By :  {message.chat.title or temp.B_LINK or 'ᴅʀᴇᴀᴍxʙᴏᴛᴢ'} \n\n🧾 <u>Your Requested Files Are Here</u> 👇 \n\n</b>"
            for idx, file in enumerate(files, start=1):
                cap += f"<b>\n{idx}. <a href='https://telegram.me/{temp.U_NAME}?start=file_{message.chat.id}_{file.file_id}'>[{get_size(file.file_size)}] {clean_filename(file.file_name)}\n</a></b>"
